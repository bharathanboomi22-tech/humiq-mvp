...
CREATE TABLE IF NOT EXISTS public.interview_results (
  ...
  -- Structure: { summary, strengths[], areasToImprove[], advice, nextSteps }
  
  -- Recap côté Company (axé sur la décision)
  company_recap JSONB NOT NULL DEFAULT '{}',
  -- Structure: { summary, fitScore, skillsAssessed[], redFlags[], recommendation }
  
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);

-- Create indexes
CREATE INDEX idx_interview_results_interview_request_id ON public.interview_results(interview_request_id);
CREATE INDEX idx_interview_results_work_session_id ON public.interview_results(work_session_id);
...
COMMENT ON TABLE public.interview_results IS 'Stores interview results with separate recaps for talent and company';
COMMENT ON COLUMN public.interview_results.talent_recap IS 'Talent-focused recap: strengths, areas to improve, advice';
COMMENT ON COLUMN public.interview_results.company_recap IS 'Company-focused recap: fit score, skills assessed, recommendation';
